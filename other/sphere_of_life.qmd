...
Genetics also consists of a lot of paramters. You have ~20,000 genes, and each gene can be expressed as RNA at a different density. Again there are sub-dimensions here, in that the RNA expression can vary spatially, but if we smudge that out how can we think about the state of cells through their genetics? An interesting (and by interesteing I tend to mean difficult and dumb) technique that geneticists use is the UMAP. UMAP (Uniform manifold approximation and projection) is a form of nonlinear dimensionality reduction, and it's the go-to method in genetics to turn many dimensions into a nice plot that fits on a piece of paper, which means two dimensions. The two dimensions are UMAP1 and UMAP2.
...
<head>
    ...
    <style>
        ...
        #canvasContainer { /* New container for canvas and overlays */
            position: relative;
            width: 100%;
            max-width: 600px; /* Ensure it doesn't exceed canvas max */
            aspect-ratio: 1 / 1; /* Maintain square aspect ratio for canvas area */
            margin-bottom: 10px;
        }
        #renderCanvas {
            display: block;
        ...
        #controls p { /* Only for the instruction paragraph now */
            margin-top: 0;
        }
        #statsMenu {
            display: none; 
...
<body>
    ...
    <div id="sphereAppContainer">
        <div id="controls">
            <center>
            <p>Use mouse to rotate. Hold WASD to move the dot. Dot also rolls with gravity.</p>
            </center>
            </div>
    ...
    <script type="module">
        ...
        let radiusHistory = [];
        let currentBaseHue = 0.66; // Initial hue (blue-ish)

        const USER_SPEED_RAD_PER_SEC = 0.3;
        const K_GRAVITY = 3.0;
        ...
        function init() {
            ...
            const ambientLight = new THREE.AmbientLight(0xdddddd, 0.9); // Softer ambient
            scene.add(ambientLight);
            const directionalLight = new THREE.DirectionalLight(0xffffff, 0.8); // Slightly softer directional
            directionalLight.position.set(1, 1.5, 1).normalize();
            scene.add(directionalLight);
            const directionalLight2 = new THREE.DirectionalLight(0xffffff, 0.4); // Slightly softer
            ...
            playerDot = createPlayerDot(); // Create dot first to apply initial hue
        ...
        function setupNewRun() {
            ...
            // Increment and wrap hue for next run
            currentBaseHue = (currentBaseHue + 0.15) % 1.0; 
            ...
            // Update sphere and dot colors based on new hue
            if (noisySphere) updateSphereGeometry(0); // Reset sphere visual state to t=0 with new hue
            if (playerDot) {
                const newDotColor = new THREE.Color().setHSL((currentBaseHue + 0.5) % 1.0, 0.9, 0.55); // Complementary-ish
        ...
        function getColorForHeight(normalizedHeight) {
            ...
            // Use currentBaseHue and vary saturation/lightness more for contrast
            const saturation = 0.6 + normalizedHeight * 0.4; // Greener/bluer at peaks
            const lightness = 0.4 + normalizedHeight * 0.3;  // Brighter at peaks
        ...
        function updateSphereGeometry(currentTimeForNoise) {
            ...
            for (let i = 0; i < positions.count; i++) {
                ...
                const normalizedHeight = (currentRadius - minRForColor) / (maxRForColor - minRForColor);
                const vertexColor = getColorForHeight(Math.max(0, Math.min(1, normalizedHeight))); // Uses currentBaseHue
        ...
        function createPlayerDot() {
            ...
            // Initial color will be set in setupNewRun
            const dotMaterial = new THREE.MeshBasicMaterial({ color: 0xffaa00 }); 
            const dotMesh = new THREE.Mesh(dotGeometry, dotMaterial);

            playerDotLight = new THREE.PointLight(0xffaa00, 2.5, baseRadius * 0.5); // Slightly reduced intensity/distance
        ...
        function applyGravity(deltaTime) {
            ...
            let acc_theta = -K_GRAVITY * grad_R_dTheta / (playerR * effectiveSinPhi);
            let acc_phi   = -K_GRAVITY * grad_R_dPhi / playerR;
            ...
            if (Math.abs(playerVelTheta) > 0 || Math.abs(playerVelPhi) > 0) {
                ...
                playerTheta = (playerTheta + 2 * Math.PI * 1000) % (2 * Math.PI); 
            }
        }

        function handleContinuousMovement(deltaTime) {
            let movedByUserInput = false;
            const userDisplacement = USER_SPEED_RAD_PER_SEC * deltaTime;
            if (keysPressed['w']) { playerPhi -= userDisplacement; movedByUserInput = true; }
            if (keysPressed['s']) { playerPhi += userDisplacement; movedByUserInput = true; }
            if (keysPressed['a']) { playerTheta -= userDisplacement; movedByUserInput = true; }
            if (keysPressed['d']) { playerTheta += userDisplacement; movedByUserInput = true; }
            if (movedByUserInput) {
                playerVelTheta = 0; playerVelPhi = 0;
                playerPhi = Math.max(0.001, Math.min(Math.PI - 0.001, playerPhi));
                playerTheta = (playerTheta + 2 * Math.PI * 1000) % (2 * Math.PI);
            }
        }
        ...
        function updateTrail(deltaTime, currentTotalTimeInSeconds) {
            ...
            const minSpeedForTrail = 0.03; 
            const isMovingByKey = keysPressed['w'] || keysPressed['s'] || keysPressed['a'] || keysPressed['d'];
            const isRollingFastEnough = speed > minSpeedForTrail && !isMovingByKey; 
            if ((isMovingByKey || isRollingFastEnough) && (currentTotalTimeInSeconds * 1000 - lastTrailUpdateTime > TRAIL_UPDATE_INTERVAL * 1000 )) {
                ...
                const trailSegmentMat = new THREE.MeshBasicMaterial({
                    color: playerDot.material.color, // Trail matches current dot color
        ...
        function onKeyDown(event) {
            ...
            if (['w', 'a', 's', 'd'].includes(key)) { keysPressed[key] = true; event.preventDefault(); }
        }
        function onKeyUp(event) {
            const key = event.key.toLowerCase();
            if (['w', 'a', 's', 'd'].includes(key)) { keysPressed[key] = false; event.preventDefault(); }
        }
        ...
        function onWindowResize() {
            ...
            // Calculate width based on appContainer, subtracting its padding
            ...
            availableWidth = Math.min(600, availableWidth); // Cap at 600px or use smaller available width
            
            // Set canvasContainer size (which dictates canvas size via 100% width/height)
            canvasContainer.style.width = availableWidth + 'px';
            canvasContainer.style.height = availableWidth + 'px'; // Maintain 1:1 aspect ratio for the container

            camera.aspect = 1; // Canvas is square
        ...
        function animate(timestamp) {
            ...
            if (physicsDeltaTime > 0) {
                handleContinuousMovement(physicsDeltaTime); 
                applyGravity(physicsDeltaTime);             
                updateTrail(physicsDeltaTime, timestamp * 0.001); 
            }
            updatePlayerDotPosition(); // This also calls updateUIDisplays

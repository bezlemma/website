...
<body>
  ...
  <div class="game-container">
    ...
    <div id="gameMessage" style="font-size: 1em; color: #ffcc88; min-height: 1.2em;">Press D/Space or Left Arrow/Enter to Serve</div>
  </div>
  ...
  <div class="controls-info">
    ...
    <p><strong>Player 2 (Right):</strong> Move: <strong>ArrowUp</strong>/<strong>ArrowDown</strong>. Push Jet: <strong>ArrowLeft</strong>/<strong>Enter</strong>. Pull: <strong>ArrowRight</strong> .</p>
  </div>
...
function movePaddles(){if(keysPressed['w'])paddleLeftY-=PADDLE_SPEED;if(keysPressed['s'])paddleLeftY+=PADDLE_SPEED;paddleLeftY=Math.max(0,Math.min(paddleLeftY,COURT_HEIGHT-PADDLE_HEIGHT));if(keysPressed['arrowup'])paddleRightY-=PADDLE_SPEED;if(keysPressed['arrowdown'])paddleRightY+=PADDLE_SPEED;paddleRightY=Math.max(0,Math.min(paddleRightY,COURT_HEIGHT-PADDLE_HEIGHT));if(serveState==='p1_serve')ball.y=paddleLeftY+PADDLE_HEIGHT/2;else if(serveState==='p2_serve')ball.y=paddleRightY+PADDLE_HEIGHT/2;ball.y=Math.max(BALL_RADIUS,Math.min(ball.y,COURT_HEIGHT-BALL_RADIUS));}
...
function update(gameDt) {
    ...
    let p2_is_pushing = (keysPressed['arrowleft'] || keysPressed['enter']) && (serveState === 'playing' || serveState === 'p2_serve');
    let p1_is_pulling = keysPressed['a'] && serveState === 'playing';
    let p2_is_pulling = keysPressed['arrowright'] && serveState === 'playing';
